...
InputManager = new function () {
	...
	this.padUpdate = function () {
		var state = 0;
		if (this.currentlyPressedKeys[this.KEY.ARROW_UP])
			state = state | this.PAD.UP;
		if (this.currentlyPressedKeys[this.KEY.ARROW_DOWN])
			state = state | this.PAD.DOWN;
		if (this.currentlyPressedKeys[this.KEY.ARROW_LEFT])
			state = state | this.PAD.LEFT;
		if (this.currentlyPressedKeys[this.KEY.ARROW_RIGHT])
			state = state | this.PAD.RIGHT;
		if (this.currentlyPressedKeys[this.KEY.SPACEBAR])
			state = state | this.PAD.OK;
		if (this.currentlyPressedKeys[this.KEY.ENTER])
			state = state | this.PAD.OK;
		if (this.mouseDown || this.mouseClick)
			state = state | this.PAD.OK;
		if (this.currentlyPressedKeys[this.KEY.ESCAPE])
			state = state | this.PAD.CANCEL;
